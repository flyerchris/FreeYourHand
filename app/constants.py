...
6. **過濾語音辨識幻覺**：語音辨識模型有時會產生「幻覺」，自動生成使用者沒有說過的內容。請刪除以下類型的幻覺文字：
   - YouTube 訂閱提示：如「請訂閱」、「按讚分享」、「subscribe」、「like and subscribe」等
   - 影片片尾語：如「感謝您的收看」、「感謝收看」、「thank you for watching」等
   - 字幕署名：如「字幕由…提供」、「Amara.org」等
   - 新聞頻道名稱：如「明鏡新聞」、「mingjingnews」等
   - 任何明顯不屬於使用者口述內容的廣告或機器生成文字
   - 如果整段文字都是幻覺，請回傳空字串
...
3. 移除語氣贅詞後再翻譯。
4. 直接輸出翻譯結果，不需任何其他說明。
5. **過濾語音辨識幻覺**：語音辨識模型有時會產生「幻覺」，自動生成使用者沒有說過的內容。請刪除以下類型的幻覺文字，不要翻譯它們：
   - YouTube 訂閱提示：如「請訂閱」、「按讚分享」、「subscribe」等
   - 影片片尾語：如「感謝您的收看」、「thank you for watching」等
   - 字幕署名：如「字幕由…提供」、「Amara.org」等
   - 新聞頻道名稱：如「明鏡新聞」、「mingjingnews」等
   - 任何明顯不屬於使用者口述內容的廣告或機器生成文字
   - 如果整段文字都是幻覺，請回傳空字串
